...
async function getRemoteAsset(url: string): Promise<Buffer> {
  ...
  if (url.startsWith('http')) {
    ...
    // Retry com backoff: o desafio anti-bot do Cloudflare (202 + HTML) e intermitente
    // sob rajada de requisicoes; costuma passar na 2a/3a tentativa.
    ...
    for (let tentativa = 1; tentativa <= MAX_TENTATIVAS; tentativa++) {
      ...
      let res: Response;
      try {
        res = await fetch(url, { signal: controller.signal, headers: browserHeaders });
      } finally {
        clearTimeout(timer);
      }
      if (res.ok) {
    ...
    throw new Error(`Falha ao buscar imagem ${url} apos ${MAX_TENTATIVAS} tentativas: ${ultimoErro}`);
  }
...
async function renderTextBlock(
  ...
  while (fontSize > minFont) {
    ...
    const lh = Math.max((layer.line_height ?? 1.2) * (fontSize / (layer.font_size || fontSize || 1)), naturalLh);
  ...
  if (maxFont > layer.font_size && fontSize === layer.font_size) {
    const cabe = (fs: number, linhas: string[]): boolean => {
      ...
      const lh = Math.max((layer.line_height ?? 1.2) * (fs / (layer.font_size || fs || 1)), nlh);
  ...
  const naturalLh = ((font.ascent + Math.abs(font.descent)) / font.unitsPerEm) * fontSize;
  const lineHeight = Math.max((layer.line_height ?? 1.2) * (fontSize / (layer.font_size || fontSize || 1)), naturalLh);

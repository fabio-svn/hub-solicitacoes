...
window.FormCore = (function () {
  ...
  async function submit(opts) {
    ...
    /* CODIGO-NA-TELA: o catch era vazio, entao queda de rede no meio do upload
       ficava indistinguivel de recusa do servidor — as duas viravam "Erro ao
    ...
    if (errEl) {
      ...
      errEl.textContent = falhaDeRede
        ? 'A conexão caiu durante o envio. Verifique a internet e tente de novo — o rascunho foi mantido.'
        : (d.error || 'Erro ao enviar. Tente novamente.') + (d.ref ? ' (código ' + d.ref + ')' : '');
    }

...
// ── Ajuda flutuante (WhatsApp + Feedback do Hub) ─────────────────────────
/* O botão do WhatsApp virou um leque: empilhar um segundo FAB no mesmo canto
   dava dois círculos sem rótulo, e no celular eles ocupavam metade da margem
   direita. A montagem toda mora no feedback-hub.js — aqui só o carregamento,
   idempotente, para não precisar tocar em ~40 arquivos HTML. */
(function () {
  function _carregarFeedbackHub() {
    if (window._svnFeedbackHubCarregado) return;
    if (document.querySelector('script[data-svn-feedback-hub]')) return;
    var s = document.createElement('script');
    s.src = '/feedback-hub.js';
    s.defer = true;
    s.setAttribute('data-svn-feedback-hub', '1');
    document.head.appendChild(s);
  }
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', _carregarFeedbackHub);
  else _carregarFeedbackHub();
})();
